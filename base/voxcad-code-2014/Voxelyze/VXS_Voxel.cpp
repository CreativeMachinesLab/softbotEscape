...
//		if ((AngMomZPos && AngMom.z < 0) || (AngMomZNeg && AngMom.z > 0)) AngMom.z = 0;
	...
	//SCALE
	if (pSim->ActuationFile != "")
	{
		if (pSim->CurTime >= pSim->ActuationMatrix[pSim->nextActuationLine][0])
		{
			 vfloat TempFact = pSim->ActuationMatrix[pSim->nextActuationLine][GetMaterialIndex()];
	...
	//		vfloat ThisTemp = pSim->pEnv->pObj->GetBaseMat(GetMaterialIndex())->GetCurMatTemp();
			// std::cout << "GetMaterialIndex: " << GetMaterialIndex() << std::endl;
