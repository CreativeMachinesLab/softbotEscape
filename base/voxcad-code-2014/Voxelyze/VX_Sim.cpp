...
//		pXML->Element("EnforceLatticeEnabled", IsFeatureEnabled(VXSFEAT_));
		pXML->Element("ActuationFile", ActuationFile);
		pXML->UpLevel();
...
//		if (pXML->FindLoadElement("Integrator", &tmpInt)) CurIntegrator = (IntegrationType)tmpInt; else CurIntegrator = I_EULER;
	...
	if (pXML->FindElement("Features")){
		...
		if (pXML->FindLoadElement("ActuationFile", &ActuationFile)) ; else ActuationFile = "";
...
//#endif
	...
	if ( ActuationFile != "" )
	{
		std::cout << "Importing Acutation Data From " << ActuationFile << std::endl;
		std::ifstream myfile(ActuationFile.c_str()); 
	    if(!myfile) //Always test the file open.
	    {
            std::cout << std::endl << "ERROR: Actuation file specified in VXA does not exist!" << std::endl;
            std::cout << "quttting." << std::endl;
            exit(0);
	    }
		...
		// std::string line;
		// for (int i = 0; std::getline(myfile, line); ++i) { number_of_lines++; }
		// boost::split(strs, line, boost::is_any_of("\t "));
  //   	std::cout << "length of file = " << number_of_lines << std::endl;
  //   	std::cout << "entries per line = " << strs.size() << std::endl;
    	myfile.close();

		std::string line;
	    std::vector<std::string> thisLineVectorString;
	    std::vector<double> thisLineVectorDouble;
    	myfile.open(ActuationFile.c_str()); 
    	for (int i = 0; std::getline(myfile, line); ++i) 
    	{ 
    		std::cout<< line << std::endl;
    		if (i==0) {continue;}
    		boost::split(thisLineVectorString, line, boost::is_any_of("\t "));

			// for(std::vector<std::string>::iterator it = thisLineVectorString.begin(); it != thisLineVectorString.end(); ++it)
			thisLineVectorDouble.clear(); 
			for (int j=0; j<thisLineVectorString.size(); j++)
			{
    			thisLineVectorDouble.push_back(atof(thisLineVectorString.at(j).c_str()));
    			std::cout<< thisLineVectorString.at(j).c_str() << " -> ";
    			std::cout<< thisLineVectorDouble.at(j) << std::endl;
			}	

    		ActuationMatrix.push_back(thisLineVectorDouble);
    		// std::cout << line << std::endl; 
    	}
    	myfile.close();
    	...
    	// 	for (int j=0; j<5; j++)	
    	// 	{
    	// 		std::cout << ActuationMatrix[i][j] << " ";
    	// 	}
    	// 	std::cout << std::endl;
    	// }

	}
	// else { std::cout << "NoActuationFile" << std::endl; }
...
bool CVX_Sim::TimeStep(std::string* pRetMessage)
{
	...
	if (ActuationFile != "")
	{
		if (CurTime > ActuationMatrix[nextActuationLine][0]+1*dt) 
		{ 
			nextActuationLine++; 
			std::cout << "CurTime: " << CurTime << ", nextActuationTime: " <<  ActuationMatrix[nextActuationLine][0] << std::endl;
		}
	}
...
bool CVX_Sim::UpdateStats(std::string* pRetMessage) //updates simulation state (SS)
{
	...
	// 	{
	// 		// std::cout << it->GetCurPos().x << std::endl;
	// 		// std::cout <<  i << ": " << abs(it->GetCurPos().x) <<  ", " << abs(it->GetCurPos().x) << ", " << std::max(abs(it->GetCurPos().x), abs(it->GetCurPos().y)) << std::endl;
	// 		if (std::max(abs(it->GetCurPos().x), abs(it->GetCurPos().y)) > SS.MaxVoxelDist)
	// 		{
	// 			SS.MaxVoxelDist = std::max(abs(it->GetCurPos().x), abs(it->GetCurPos().y));
	...
	if (CDisp || CVel || CKinE || CPressure){
		int nVox = NumVox(); // nac: initialized above in MaxVoxelDist calc
