    ...
    void GeneticPopulation::speciate()
    {
        ...
        for (int a=0;a<generations[generations.size()-1/*onGeneration*/]->getIndividualCount();a++)
        {
            ...
            for (int b=0;b<(int)species.size();b++)
            {
                ...
                cout << "fitness: " << individual->getFitness() << ", voxels: " << individual->getFitness2() << ", compatibility: " << compatibility << endl;
                if (compatibility<compatThreshold)
                {
                    //Found a compatible species
                    individual->setSpeciesID(species[b]->getID());
                    cout << "fits species " << species[b]->getID() << " (with champ fitness: " << species[b]->getBestIndividual()->getFitness() << ", voxels: " << species[b]->getBestIndividual()->getFitness2() << ")" << endl;
            ...
            if (makeNewSpecies)
            {
                cout << "new species from inviduual:: fitness: " << individual->getFitness() << ", voxels: " << individual->getFitness2() << endl;
