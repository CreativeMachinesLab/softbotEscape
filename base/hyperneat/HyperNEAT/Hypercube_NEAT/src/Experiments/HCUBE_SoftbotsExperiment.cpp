...
namespace HCUBE
{
    ...
    :   Experiment(_experimentName)
    {
    	...
    	if ( NEAT::Globals::getSingleton()->getTemplateFileName() != "" )
    	{

			templatefile.open( NEAT::Globals::getSingleton()->getTemplateFileName().c_str() ); // this is the name of the fitness file created (as we specified it when writing the vxa)
			...
			{
				int z=0;
				// templateMatrix(num_x_voxels, std::vector<std::vector<int> >(num_y_voxels, std::vector<int>(num_z_voxels, 0)));
				while (std::getline(templatefile, line))
				{
				    std::size_t foundx = line.find("<X_Voxels>");
				    if (foundx!=std::string::npos)
				    {
				    	num_x_voxels = atof(line.substr(foundx+strlen("<X_Voxels>"),line.find("</")-(foundx+strlen("<X_Voxels>"))).c_str());
				    	NEAT::Globals::getSingleton()->setParameterValue("BoundingBoxX",num_x_voxels);
				    }
				    std::size_t foundy = line.find("<Y_Voxels>");
				    if (foundy!=std::string::npos)
				    {
				    	num_y_voxels = atof(line.substr(foundy+strlen("<Y_Voxels>"),line.find("</")-(foundy+strlen("<Y_Voxels>"))).c_str());
				    	NEAT::Globals::getSingleton()->setParameterValue("BoundingBoxY",num_y_voxels);
				    }
				    std::size_t foundz = line.find("<Z_Voxels>");
				    if (foundz!=std::string::npos)
				    {
				    	num_z_voxels = atof(line.substr(foundz+strlen("<Z_Voxels>"),line.find("</")-(foundz+strlen("<Z_Voxels>"))).c_str());
				    	NEAT::Globals::getSingleton()->setParameterValue("BoundingBoxZ",num_z_voxels);
 
 						vector< int > tmpVectorZ(num_z_voxels,0);
 						vector< vector< int > > tmpVectorY(num_y_voxels,tmpVectorZ);
 						vector< vector< vector< int > > > tmpVectorX(num_x_voxels,tmpVectorY);
 						templateMatrix = tmpVectorX;
						// templateMatrix.resize(num_x_voxels);
						// templateMatrix[0].resize(num_y_voxels);
						// templateMatrix[0][0].resize(num_z_voxels);
				    }

				    std::size_t foundl = line.find("<Layer>");
				    if (foundl!=std::string::npos)
				    {
				    	std::string layer = line.substr(foundl+strlen("<Layer><![CDATA["),line.find("]]></Layer>")-(foundl+strlen("<Layer><![CDATA["))); 
				    	...
				    	for (int y=0; y<num_y_voxels; y++)
				    	{
				    		...
				    		{				    			
				    			// cout << x << "," << y << "," << z << std::endl;
				    			// cout << x << "," << y << "," << z << ": orig: " << layer[x+num_x_voxels*y] << std::endl;
				    			// std::string tmpStr = std::string(layer[x+num_x_voxels*y]);
				    			// cout << tmpStr << endl;
				    			templateMatrix[x][y][z] = atoi(layer.substr(x+num_x_voxels*y,1).c_str());
				    			// cout << x << "," << y << "," << z << ": " << templateMatrix[x][y][z] << std::endl;
				    		}
				    	}
				    	z++;
				    }
    ...
    double SoftbotsExperiment::processEvaluation(shared_ptr<NEAT::GeneticIndividual> individual)
    {
		...
		fitness2 = 0.000001; // worst fitness value (higher is better, max is 1)
    	...
    	// for appature only, set bounding box to full size
    	...
    	for (std::map<Coords,int>::iterator it=cppnOutput.begin(); it!=cppnOutput.end(); ++it)
		{
    		// if ( NEAT::Globals::getSingleton()->getTemplateFileName() != "" or templateMatrix[it->first.x-minIndexX][it->first.y-minIndexY][it->first.z-minIndexZ] == 0)
    		// {
    			matrixForVoxelyze[it->first.x-minIndexX][it->first.y-minIndexY][it->first.z-minIndexZ] = it->second;
    		// }
    	}

  //   	cout << "Template Matrix" << endl;
  ...
  //   	cout << endl << "Matrix For Voxelyze" << endl;
  //   	for (int z=0; z<num_z_voxels; z++)
  //   	{
  //   		for (int y=0; y<num_y_voxels; y++)
  //   		{
  ...
  //   	vector<Coords> allVoxelsNotChecked;
  //   	for (int z=0; z<num_z_voxels; z++)
  //   	{
  //   		for (int y=0; y<num_y_voxels; y++)
  //   		{
  //   			for (int x=0; x<num_x_voxels; x++)
  //   			{
  //   				allVoxelsNotChecked.push_back(Coords(x,y,z));
  //   			}
  //   		}
  //   	}

  //   	cout << endl << "New Matrix -- multiplePatches" << endl;
  //   	for (int z=0; z<num_z_voxels; z++)
  //   	{
  //   		for (int y=0; y<num_y_voxels; y++)
  //   		{
  ...
  //   	// vector< vector< vector< int > > > newMatrixForVoxelyze (num_x_voxels, std::vector<std::vector<int> >(num_y_voxels, std::vector<int>(num_z_voxels, 0)));

  //   	// queue<Coords> queueToCheck;
  //   	while ( !queueToCheck.empty() ) { queueToCheck.pop(); }
  //   	vector<Coords> queueBiggestPatch;
  //   	vector<Coords> queueCurrentPatch;

		// // std::map<Coords,int>::iterator it;

		// // std::map<Coords,int> alreadyChecked;
		// // std::map<Coords,int> cppnZeros;
		...
		// // while voxels filled < max and queue is not empty:
		// while ( allVoxelsNotChecked.size() > biggestPatch )
		// {
		// 	queueToCheck.push( allVoxelsNotChecked.back() );
		// 	// allVoxelsNotChecked.pop_back();
			
		// 	while ( not queueToCheck.empty() )
		// 	{
		// 		thisCoord = queueToCheck.front();
		// 		queueToCheck.pop();
		// 		if ( find(allVoxelsNotChecked.begin(), allVoxelsNotChecked.end(), thisCoord) != allVoxelsNotChecked.end() ) 
		// 		{
		// 			allVoxelsNotChecked.erase( find(allVoxelsNotChecked.begin(), allVoxelsNotChecked.end(), thisCoord) ); // if (it != myvector.end())
		// 		}

		// 		if ( matrixForVoxelyze[thisCoord.x][thisCoord.y][thisCoord.z] > 0 and templateMatrix[thisCoord.x][thisCoord.y][thisCoord.z] > 0 )
		// 		{
		// 			queueCurrentPatch.push_back(thisCoord);

		// 			for (int i=-1; i<2; i=i+2)
		// 			{	
		// 				newCoord = Coords( thisCoord.x, thisCoord.y, int(max(min(thisCoord.z + i,float(num_z_voxels-1)),float(0.0))) );
		// 				if ( std::find(allVoxelsNotChecked.begin(), allVoxelsNotChecked.end(), newCoord) != allVoxelsNotChecked.end() ) { queueToCheck.push(newCoord); } // if the voxel hasn't been checked yet, add it to queue
		// 			}
		// 		}
		...
		// 			for (int i=-1; i<2; i=i+2)
		// 			{	
		// 				newCoord = Coords( thisCoord.x, int(max(min(thisCoord.y + i,float(num_y_voxels-1)),float(0.0))), thisCoord.z );
		// 				if ( std::find(allVoxelsNotChecked.begin(), allVoxelsNotChecked.end(), newCoord) != allVoxelsNotChecked.end() ) { queueToCheck.push(newCoord); } // if the voxel hasn't been checked yet, add it to queue
		// 			}
		// 		}
		...
		// 			for (int i=-1; i<2; i=i+2)
		// 			{	
		// 				newCoord = Coords( int(max(min(thisCoord.x + i,float(num_x_voxels-1)),float(0.0))), thisCoord.y, thisCoord.z );
		// 				if ( std::find(allVoxelsNotChecked.begin(), allVoxelsNotChecked.end(), newCoord) != allVoxelsNotChecked.end() ) { queueToCheck.push(newCoord); } // if the voxel hasn't been checked yet, add it to queue
		// 			}
		// 		}					
		// 	}

		// 	if ( queueCurrentPatch.size() > biggestPatch )
		// 	{
		// 		biggestPatch = queueCurrentPatch.size();
		// 		queueBiggestPatch = queueCurrentPatch;
		// 		cout << "new biggest patch size: " << queueBiggestPatch.size() << endl;
		// 	}
		// 	// while ( !queueCurrentPatch.empty() ) { queueCurrentPatch.erase(0);(); }
		// 	queueCurrentPatch.clear();
		// }



		// // nac TODO: make into "newMatrix"
		// vector< vector< vector< int > > > newMatrixForVoxelyze (num_x_voxels, std::vector<std::vector<int> >(num_y_voxels, std::vector<int>(num_z_voxels, 0)));
			
		// while ( not queueBiggestPatch.empty() )
		// {
		// 	newMatrixForVoxelyze[queueBiggestPatch.back().x][queueBiggestPatch.back().z][queueBiggestPatch.back().y] = matrixForVoxelyze[queueBiggestPatch.back().x][queueBiggestPatch.back().z][queueBiggestPatch.back().y];
		// 	queueBiggestPatch.pop_back();
		// }

		// matrixForVoxelyze = newMatrixForVoxelyze;
		// cout << endl << "New Matrix For Voxelyze" << endl;
  //   	for (int z=0; z<num_z_voxels; z++)
  //   	{
  //   		for (int y=0; y<num_y_voxels; y++)
  //   		{
  //   			for (int x=0; x<num_x_voxels; x++)
  //   			{
  //   				cout << newMatrixForVoxelyze[x][y][z];
  //   			}
  //   		}
  //   		cout << endl;
  //   	}
		...
		if (fitnessLookup.find(md5sumString) != fitnessLookup.end())
		{
			...
			if ( NEAT::Globals::getSingleton()->getParameterValue("NoveltyNeighborhood") > 0 )
			{
				pair<double, double> endPosBoth = endPosArchive[md5sumString];
				endPosX = endPosBoth.first;
				endPosY = endPosBoth.second;
			}
		...
		else
		{
			...
			while (not doneEval)
			{
				...
				else
				{
					...
					if ( double(end-start)/CLOCKS_PER_SEC > 1000.0) // amount of time set arbitrarily.  For a more scalable value, make function of the number of voxels
					{
						cout << "voxelyze hung after 1000 seconds... assigning fitness of 0.000001"<<endl;
			...
			if (infile.is_open())
			{
				while (std::getline(infile, line))
				{
			    	...
			    	// std::size_t foundx = line.find("<FinalFitness>");
				    // if (foundx!=std::string::npos)
				    // {
				    // 	fitness = atof(line.substr(foundx+strlen("<FinalFitness>"),line.find("</")-(foundx+strlen("<FinalFitness>"))).c_str());
				    // }

				    std::size_t foundx = line.find("<EndPosX>");
				    if (foundx!=std::string::npos)
				    {
				    	endPosX = atof(line.substr(foundx+strlen("<EndPosX>"),line.find("</")-(foundx+strlen("<EndPosX>"))).c_str());
				    }

				    std::size_t foundy = line.find("<EndPosY>");
				    if (foundx!=std::string::npos)
				    {
				    	endPosY = atof(line.substr(foundx+strlen("<EndPosY>"),line.find("</")-(foundx+strlen("<EndPosY>"))).c_str());
				    }
		...
		if ( NEAT::Globals::getSingleton()->getParameterValue("NoveltyNeighborhood") > 0 )
		{
			if ( endPosArchive.size() < int(NEAT::Globals::getSingleton()->getParameterValue("NoveltyNeighborhood")) )
			{
				fitness = 10000000;  // if nothing to compare to, automatically super novel! (or also comare novelty among these???)
				pair <double, double> endPosBoth (endPosX, endPosY);	
				endPosArchive[md5sumString]=endPosBoth;
			} 
			else
			{
				std::vector<double> neightborhood;
				// for (int i=0; i<endPosArchive.size(); i++)
				// {

				// }
				for(auto it = neightborhood.begin(); it != neightborhood.end(); i++)
				{
				  it->??
				}
			}
	...
	void SoftbotsExperiment::moveFitnessFile( shared_ptr<const NEAT::GeneticIndividual> individual )
	{
		...
		// if (NEAT::Globals::getSingleton()->getParameterValue("SaveAllChampVXAs") and (
		// 	( int(NEAT::Globals::getSingleton()->getParameterValue("PenaltyType")) == 0 and fitness > getBestFitnessThisGen() ) or 
		// 	( NEAT::Globals::getSingleton()->getParameterValue("PenaltyType") > 0 and individual->getDominatedBy() < 1) ) )
		{
...
<X_Voxels>" << num_x_voxels+4 << "</X_Voxels>\n\
<Y_Voxels>" << num_y_voxels+4 << "</Y_Voxels>\n\
...
{
	// make front
	for (int x=0; x<num_x_voxels+4; x++) 
	{
		// make hole for appature
		if ( pow( pow( float(x) - float(num_x_voxels+4)/2.0, 2.0 ) + pow( float(z)+1 - float(num_z_voxels)/2.0, 2.0 ), 0.5 ) < NEAT::Globals::getSingleton()->getParameterValue("ApertureSize") )
		{myfile << 0;}
		else 
		{myfile << 5;}
	}
	// make front gap
	myfile << 5;
	for (int x=0; x<num_x_voxels+2; x++) 
	{
		myfile << 0;
	}
	myfile << 5;
	...
	for (int y=0; y<num_y_voxels; y++)
	{
		// make left side
		// make hole for appature
		if (pow ( pow( float(y) - float(num_y_voxels)/2.0, 2.0 ) + pow( float(z)+1 - float(num_z_voxels)/2.0, 2.0 ), 0.5 ) < NEAT::Globals::getSingleton()->getParameterValue("ApertureSize") )
		{myfile << 0;}
		else 
		{myfile << 5;}
		// make left gap
...
// 	{
// 		// make hole for appature
// 		if ( abs(x)+abs(y) < NEAT::Globals::getSingleton()->getParameterValue("ApertureSize") )
// 		{myfile << 0;}
// 		else 
// 		{myfile << 5;}
// 	}
