...
int HyperNEAT_main(int argc,char **argv)
{
	...
	else if(commandLineParser.HasSwitch("-I") && commandLineParser.HasSwitch("-O"))
	{
		...
		if(commandLineParser.HasSwitch("-C")){
			string templateFile = commandLineParser.GetSafeArgument("-C",0,"");
			NEAT::Globals::getSingleton()->setTemplateFileName(templateFile); //set the name of the custom template file to definte a custom bounding box
		} else {
			NEAT::Globals::getSingleton()->setTemplateFileName(""); //set default
		}
