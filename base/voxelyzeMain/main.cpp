...
int main(int argc, char *argv[])
{
	...
	Simulator.minCageX = 999;
	Simulator.maxCageX = -999;
	Simulator.minCageY = 999;
	Simulator.maxCageY = -999;
	int nVox = Simulator.NumVox();
	for (int i=0; i<nVox; i++)
	{
		CVXS_Voxel* it = &Simulator.VoxArray[i];
		if (it->GetMaterialIndex() == 5)
		{
			if (it->GetCurPos().x < Simulator.minCageX) { Simulator.minCageX = it->GetCurPos().x; }
			if (it->GetCurPos().y < Simulator.minCageY) { Simulator.minCageY = it->GetCurPos().y; }
			if (it->GetCurPos().x > Simulator.maxCageX) { Simulator.maxCageX = it->GetCurPos().x; }
			if (it->GetCurPos().y > Simulator.maxCageY) { Simulator.maxCageY = it->GetCurPos().y; }
		}
	}
	// std::cout << "minCageX: " << Simulator.minCageX << std::endl;
	// std::cout << "maxCageX: " << Simulator.maxCageX << std::endl;
	// std::cout << "minCageY: " << Simulator.minCageY << std::endl;
	// std::cout << "maxCageY: " << Simulator.maxCageY << std::endl;
	// exit(0);
